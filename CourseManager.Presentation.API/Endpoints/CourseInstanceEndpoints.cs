...
public static class CourseInstancesEndpoints
{
    public static IEndpointRouteBuilder MapCourseInstanceEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/{id:int}", async (int id, CourseInstanceService service, CancellationToken ct) =>
        {
            var ci = await service.GetByIdAsync(id, ct);
            return Results.Ok(ci);
        });

        group.MapPut("/{id:int}", async (int id, UpdateCourseInstanceRequest request, CourseInstanceService service, CancellationToken ct) =>
        {
            await service.UpdateAsync(id, request, ct);
            return Results.NoContent();
        });

        group.MapDelete("/{id:int}", async (int id, CourseInstanceService service, CancellationToken ct) =>
        {
